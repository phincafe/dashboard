// frontend/src/components/MonthlyRefunds.jsx
...
function MonthlyRefunds() {
  const [month, setMonth] = useState(currentMonthISO);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");
  const [data, setData] = useState(null);

  async function fetchMonthlyRefunds() {
    setLoading(true);
    setError("");
    try {
      const url = new URL("/api/refunds/monthly", API_BASE_URL);
      ...
      if (!res.ok) {
        ...
        throw new Error(body.error || "Failed to fetch monthly refunds");
      }

      const json = await res.json();
      setData(json);
    } catch (err) {
      setError(err.message || "Error loading monthly refunds");
      setData(null);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      >
        ...
        >
          {loading ? "Loading…" : "Load monthly refunds"}
        </button>
      </div>
          ...
          >
            ...
            >
              ...
              <div style={{ fontSize: 11, color: "#fdba74", marginTop: 4 }}>
                Sum of all refunds in this month
              </div>
            </div>
          ...
          <div style={{ marginTop: 16 }}>
            ...
            >
              ...
              {locations.length === 0 && (
                ...
                >
                  No refunds found for this month.
                </div>
              )}
            </div>
          </div>
