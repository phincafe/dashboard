...
function HourlyHeatmap() {
  ...
  // data.hourly is an OBJECT keyed by hour: {5: {...}, 6: {...}, ...}
  const hourlyObj = data?.hourly || {};
  const max = data?.maxHourAllLocations || 0;

  // Convert object → sorted array [{ hour, ...bucket }]
  const hourly = Object.keys(hourlyObj)
    .map((key) => {
      const h = Number(key);
      ...
      {data && (
        <div style={{ fontSize: 14 }}>
          ...
          <div style={{ fontSize: 13, color: "#9ca3af", marginBottom: 4 }}>
            Sales by hour (5 AM – 8 PM, all locations)
          </div>
