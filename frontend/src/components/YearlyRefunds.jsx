// frontend/src/components/YearlyRefunds.jsx
...
function YearlyRefunds() {
  ...
  async function fetchYearlyRefunds() {
    setLoading(true);
    setError("");
    try {
      const url = new URL("/api/refunds/yearly", API_BASE_URL);
      ...
      if (!res.ok) {
        ...
        throw new Error(body.error || "Failed to fetch yearly refunds");
      }

      const json = await res.json();
      setData(json);
    } catch (err) {
      setError(err.message || "Error loading yearly refunds");
      setData(null);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      >
        ...
        >
          {loading ? "Loading…" : "Load yearly refunds"}
        </button>
      </div>
      ...
      {data && (
        <>
          ...
          >
            ...
            >
              <div style={{ fontSize: 12, color: "#9ca3af" }}>Year range</div>
            ...
            >
              ...
              <div style={{ fontSize: 11, color: "#fdba74", marginTop: 4 }}>
                Sum of all refunds in this year
              </div>
            </div>
          ...
          <div style={{ marginTop: 16 }}>
            ...
            >
              ...
              {locations.length === 0 && (
                ...
                >
                  No refunds found for this year.
                </div>
              )}
            </div>
          </div>
